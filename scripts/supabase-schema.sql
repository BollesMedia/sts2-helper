...
\restrict IwlfdZLimlXYPmRwOI6mAahZugeK4WJXCZOpGgvFTcKkHYv48UzsRo2o6dwcgMr
...
ALTER TABLE ONLY public.act_paths
    ADD CONSTRAINT act_paths_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);
...
ALTER TABLE ONLY public.choices
    ADD CONSTRAINT choices_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);
...
ALTER TABLE ONLY public.evaluations
    ADD CONSTRAINT evaluations_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);
...
CREATE POLICY "Authenticated insert usage" ON public.usage_logs FOR INSERT WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY "Public read act_paths" ON public.act_paths FOR SELECT USING (true);
...
CREATE POLICY "Public read game_versions" ON public.game_versions FOR SELECT USING (true);
...
CREATE POLICY "Public read potions" ON public.potions FOR SELECT USING (true);
...
CREATE POLICY "Public read strategies" ON public.character_strategies FOR SELECT USING (true);
...
CREATE POLICY "Public read weight rules" ON public.weight_rules FOR SELECT USING (true);
...
CREATE POLICY "Users can insert own act_paths" ON public.act_paths FOR INSERT WITH CHECK ((user_id = auth.uid()));
...
CREATE POLICY "Users can insert own choices" ON public.choices FOR INSERT WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY "Users can view own act_paths" ON public.act_paths FOR SELECT USING ((user_id = auth.uid()));
...
CREATE POLICY "Users insert own errors" ON public.error_logs FOR INSERT WITH CHECK (true);
...
CREATE POLICY "Users read own errors" ON public.error_logs FOR SELECT USING ((auth.uid() = user_id));
...
ALTER TABLE public.act_paths ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.character_strategies ENABLE ROW LEVEL SECURITY;
...
\unrestrict IwlfdZLimlXYPmRwOI6mAahZugeK4WJXCZOpGgvFTcKkHYv48UzsRo2o6dwcgMr
